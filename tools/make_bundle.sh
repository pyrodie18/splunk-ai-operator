...
OPERATOR_IMAGE="$DOCKER_IO_PATH/splunk-ai-operator:${VERSION}"
...
for crd in config/crd/bases/*_crd.yaml; do
  ...
  if [ "$crd" != config/crd/bases/ai.splunk.com_monitoringconsoles_crd.yaml ]
  then
...
- command: update
  ...
  value: |-
    [{
      ...
      "metadata": {
        "name": "example",
        "finalizers": [ "ai.splunk.com/delete-pvc" ]
      },
      "spec": {
        "replicas": 1
      }
